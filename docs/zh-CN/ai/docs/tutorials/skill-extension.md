...
> 本篇由 [2.5 模式与模型选型技巧](mode-model-selection) 原「技巧四」独立而来。
...
平台对此有原生支持：创建 Skill 时选择「搜索 Skill」,可**联网搜索 GitHub 上的社区 Skill 直接导入**(见 [1.7.2 Skill 体系 · 三种创建方式](../basics/capabilities/skill));搜不到现成 Skill 的，再按下面的流程自己改造。
...
3. **改**:按 Skill 写法重构——结论前置、规则可执行、附示例 (写法参考 [2.1 提示词编写技巧](prompt-writing) 与 [1.7.2 Skill 体系](../basics/capabilities/skill));删掉与平台无关的工具指令，金融场景剔除荐股/收益承诺类内容并补上合规红线 (见 [3.1 合规要求](../compliance/index))
...
- [1.7.2 Skill 体系](../basics/capabilities/skill) — Skill 的创建 (含搜索导入)、写法与挂载
- [2.1 提示词编写技巧](prompt-writing) — Skill 内容的写法同提示词
- [3.1 合规要求](../compliance/index) — 面向客户 Agent 的内容合规
