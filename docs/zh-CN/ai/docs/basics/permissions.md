...
Agent 的编辑权限跟随其所属 Workspace 的成员角色 (详见 [Workspace 与套餐体系](plans)):
...
Workspace 拥有者可在成员管理页把成员在 **Editor / Viewer** 之间切换 (见 [主要页面介绍](pages-overview))。
...
- 发布到广场的内容审核与安全要求见 [合规要求](../compliance/index)
...
- **功能继承**:使用者能用到的功能由 **Agent 所属 Workspace 的套餐**决定，与使用者自己的套餐无关 (使用者套餐只影响用量额度)—— 详见 [配置时 vs 运行时](plans)
...
- **用量承担**:使用者是否为 Agent 所属 Workspace 成员，决定了用量记在谁头上 (开发者付费 / 使用者付费),见 [谁来承担用量](plans)

## 使用范围红线

无论权限如何配置，以下约定始终有效 (详见 [分地区服务说明](regions)):
