---
sidebar_position: 6
title: "Regional Services"
---

> The platform serves multiple regions, each operated by an **independent legal entity** and governed by local laws and regulations. This article explains what directly affects you: who your service entity is, how prices and currency are determined, how regions differ, and the **scope of use for Agents**.

## Agent Scope-of-Use Statement

> **Please understand the following terms before using the platform:**

1. **Agents you create are for your personal use only**; you may not provide commercial services to others in any form.
2. **Agents in the Agent Marketplace are for learning and exchange only**: the listing, display, use, and Fork of Marketplace Agents **do not constitute any commercial activity**, nor **any recommendation by the platform or the creator** (including but not limited to any investment advice or product recommendation).
3. All AI-generated content on the platform is labeled per the requirements of the applicable region; for the unified constraints on financial content, see [Compliance Requirements](../compliance/index).

## Service Regions Overview

| Region | Service Entity | Client | Status |
|------|---------|--------|------|
| **Hong Kong (HK)** | Longbridge AI Technologies Limited | LBHK App + Web | Live |
| **Singapore (SG)** | Longbridge AI Technologies Pte. Ltd | LBSG App + Web | Live |

- The unified Web entry point is longbridge.com/ai; the system automatically routes you to the corresponding service entity based on your **registration region**—manual switching is not supported
- The product UI and user agreements indicate your service provider (e.g. HK users see "Provided by Longbridge AI Technologies Limited"); the Usage page also shows which entity collected each charge (charged by)

## Pricing, Currency, and Payment

| Channel | Platform | Subscription Type |
|------|-----|---------|
| Apple IAP | iOS | Monthly recurring |
| Stripe | Android / Web | Monthly recurring |

- **Fixed local pricing** is used, not automatic exchange-rate conversion: HK is displayed and charged in HKD, SG in SGD
- Reference pricing (the in-product plan page's live display is authoritative): Pro about HK$148 / S$24.98 per month, Premium about HK$748 / S$128.98 per month
- **Taxes**: prices shown to SG users are **tax-exclusive**; GST (9%) is **collected separately** by the payment channel at payment time, so the actual amount paid will be higher than the displayed price. HK has no sales tax. Tax rates and policies may change with local regulations—**actual taxation prevails**
- **Currency is locked at first purchase**; to change it, unsubscribe and purchase again
- A Workspace can have only one active subscription channel at a time

## Regional Differences and Isolation

1. **Data localization**: data is stored in compliant data centers permitted for your region
2. **Entities cannot be switched**: your service entity is determined by your registration region and cannot be switched across regions (e.g. an HK user cannot switch to the SG entity)
3. **Capabilities may differ**: some models and tools are restricted by regional regulations, so available capabilities may vary by region
4. **Region-specific client distribution**: the LBHK / LBSG Apps are distributed through their respective regional app stores
5. **A globally unified compliance baseline**: all regions follow core constraints such as "no investment advice"—just follow the platform's unified compliance requirements when building Agents, with no need to differentiate by region

## FAQ Quick Reference

| Your Question | Answer |
|---------|------|
| The price I see differs from someone else's? | Pricing is fixed locally per region; SG page prices are tax-exclusive, with GST collected separately by the payment channel |
| Why was I charged more than the page price? | In SG, GST is collected separately by the payment channel at payment time; actual taxation prevails |
| Can I switch regions if I travel or move? | No—your service entity is fixed by your registration region |
| Can I offer my Agent commercially to others? | No—Agents are for personal use only and may not be provided as a commercial service |
| Are Marketplace Agents officially recommended? | No—Marketplace Agents are for learning and exchange only and do not constitute any recommendation or commercial activity |
| Why is a certain model/tool unavailable to me? | It may be restricted by regional regulations; available capabilities differ by region |
| Where is my data stored? | In compliant data centers permitted for your region |
